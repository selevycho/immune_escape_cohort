...
for cohort, min_mut in [("brca", MIN_MUT_BRCA), ("ov", MIN_MUT_OV)]:
    ...
    # Restrict to patients carrying all three data types. Line 1 needs only
    # mutations and expression, but the convergence analysis also needs copy
    # number, and computing Line 1 on a wider set would mean two slides
    ...
    usable = have_expr & cna_samples
    print(f"  samples with expression  {len(have_expr):>8}")
    print(f"  samples with copy number {len(cna_samples):>8}")
